...
def setup_training(self):
    ...
    """
    self.learnprob = 0.05 #theroy said about one step per game so i think this should be okay (0-2 steps in early (strongly randomized games) and maximum about 20)
    ...
    if not os.path.isfile("my-saved-model.pt"):     
        self.w_h1= init_weights((486, 512))
        self.w_h2 = init_weights((512, 512))
        self.w_h3 = init_weights((512, 512))
        self.w_o = init_weights((512, 6))
        self.a1 = init_alpha((512,))
        self.a2 = init_alpha((512,))
        self.a3 = init_alpha((512,))
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]): 
    ...
    with open('after_5_h_ch.txt', 'a') as file:
...
def reward_from_events(old_game_state, self, events: List[str]) -> int:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = {
        e.CRATE_DESTROYED: 10,#0.25*(3+np.exp(-step*0.2)),
        e.COIN_COLLECTED: 30,#0.5*(1+np.exp(-step*0.2)),
        e.KILLED_OPPONENT: 30,
        e.GOT_KILLED: -40,
        e.COIN_FOUND: 10,#0.25*(1+np.exp(-step*0.2)),
        e.KILLED_SELF: -30,
        e.MOVED_LEFT: 1,
        e.MOVED_RIGHT: 1,
        e.MOVED_UP: 1,
