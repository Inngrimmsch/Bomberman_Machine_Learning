...
def state_to_features(self, game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    end_vis_x = self.x+5   #plus 5 because when slicing the end is excluded
    ...
    bomb_targets = crate + close_others
...
def Neural_network_model (self, features):   #input features, not states!!       #added .clone() everywhere before to make sure theres no in-place operation here
    ...
    conv1 = conv2d(features, self.conv_w1)
    conv1 = leaky_relu(conv1)
    drop1 = dropout(conv1, self.p_dropout_hidden)

    conv2 = conv2d(drop1, self.conv_w2)
    conv2 = leaky_relu(conv2)
    drop2 = dropout(conv2, self.p_dropout_hidden)       #look at this again
