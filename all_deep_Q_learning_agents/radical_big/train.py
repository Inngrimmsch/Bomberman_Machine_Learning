...
def setup_training(self):
    ...
    if not os.path.isfile("my-saved-model.pt"):  
        ...
        self.w_so = init_weights((25, 25)) #5 outputs (4 movement and one wait) 
        self.w_t1 = init_weights((5, 25))
        self.at1 = init_alpha((25,))
        self.w_t2 = init_weights((25, 25))
        self.at2 = init_alpha((25,))
        self.w_to = init_weights((25, 25)) #5 outputs (4 movement and one bomb)
        self.w_c1 = init_weights((4, 20))
        self.ac1 = init_alpha((20,))
        self.w_c2 = init_weights((20, 20))
        self.ac2 = init_alpha((20,))
        self.w_co = init_weights((20, 20)) #4 outputs (only movement)
        self.w_d1 = init_weights((70, 140))
        self.ad1 = init_alpha((140,))
        self.w_d2 = init_weights((140, 210))
        self.ad2 = init_alpha((210,))
        self.w_d3 = init_weights((210, 210))
        self.ad3 = init_alpha((210,))
        self.w_do = init_weights((210, 6)) #the output of the decision network obviously has to be six (all possible actions)
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if best_bomb_cond(self, self_action, old_game_state):
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]): 
    ...
    self.optimizer = optim.Adam([self.w_s1, self.as1, self.w_s2, self.as2, self.w_so, self.w_t1, self.at1, self.w_t2, self.at2, self.w_to, self.w_c1, self.ac1, self.w_c2, self.ac2, self.w_co, self.w_d1, self.ad1, self.w_d2, self.ad2, self.w_d3, self.ad3, self.w_do], lr = self.alpha)
    ...
    # Store the model 
    self.model = {'ws1': self.w_s1, 'as1': self.as1, 'ws2': self.w_s2, 'as2': self.as2, 'wso': self.w_so, 'wt1': self.w_t1, 'at1': self.at1, 'wt2': self.w_t2, 'at2': self.at2, 'wto': self.w_to, 'wc1': self.w_c1, 'ac1': self.ac1, 'wc2': self.w_c2, 'ac2': self.ac2, 'wco': self.w_co, 'wd1': self.w_d1, 'ad1': self.ad1, 'wd2': self.w_d2, 'ad2': self.ad2, 'wd3': self.w_d3, 'ad3': self.ad3, 'wdo': self.w_do}
...
def reward_from_events(old_game_state, self, events: List[str]) -> int:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = { ###change all of these
        e.SHORTEST_PATH: 50,
        #e.SURVIVED_ROUND: 30,
        e.KILLED_OPPONENT: 10,#change this when training in classic scenario
        e.COIN_COLLECTED: 10,#*self.ccc,
        e.HIGHEST_DESITY: 30,
        e.BEST_BOMB: 20,
        ...
        #e.MOVED_UP: 1,
        #e.MOVED_DOWN: 1,
        e.WAITED: -10,#*self.wc, #obviously waiting isnt inherently bad but i had the problem that agents just didnt move at all
        #e.REPEATING_ONE_MOVE: -50 * self.rmp, #repeating move punishment
        #e.BACK_AND_FORTH: -30 * self.bfc,
        e.INVALID_ACTION: -3,#*self.iac, #this should be redundant because i made invalid actions impossible
        e.PLACED_BAD_BOMB: -100,
        e.GOT_KILLED: -50,
        e.KILLED_SELF: -400,
    }
    reward_sum = 0
    ...
    return reward_sum
def best_bomb_cond(self, self_action, old_game_state):
    bomb_direction = state_to_features(self, old_game_state)[1]
    self.bestbombdir = np.argmax(bomb_direction)
    if self.bestbombdir == 0 and self_action == 'UP':
        return True
    if self.bestbombdir == 1 and self_action == 'RIGHT':
        return True
    if self.bestbombdir == 2 and self_action == 'DOWN':
        return True
    if self.bestbombdir == 3 and self_action == 'LEFT':
        return True
    if self.bestbombdir == 4 and self_action == 'WAIT':
        return True
...
def shortest_path_condition(self, self_action, old_game_state):
    ...
    self.shortest_path = np.argmax(self.shortest_path)
...
def placed_bad_bomb_condition(self, self_action, new_game_state):
    if self_action == 'BOMB':
        if self.badbomb == 1 or self.bomb_score == 0:
            return True
