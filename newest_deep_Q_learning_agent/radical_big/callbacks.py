...
def state_to_features(self, game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    if close_danger_map[3][3] !=0: #if on danger
        ...
        #some idea to make it even easier
        index = np.argmin(steps_to_safety)

        steps_to_safety = np.array([0, 0, 0, 0, 0])
        steps_to_safety[index] = 1
    else: #if in safety
        
        self.danger = 0
        steps_to_safety = np.array([1, 1, 1, 1, 1])
        #oben
        if close_danger_map[3][2] != 0:
            steps_to_safety[0] = -1
        #rechts
        if close_danger_map[4][3] != 0:
            steps_to_safety[1] = -1
        #unten
        if close_danger_map[3][1] != 0:
            steps_to_safety[2] = -1
        #links
        if close_danger_map[2][3] != 0:
            steps_to_safety[3] = -1
    ...
    end_vis_x = self.x+7   #plus 7 because when slicing the end is excluded
        ...
        for y in range(3, 8):
            for xe in range(x-3, x+4):
                if field_around_agent[xe][y] == - 1:
                    break
                elif field_around_agent[xe][y] == 1:
            ...
            for ye in range(y-3, y+4): #doesnt matter that we count xy twice cause there cant be a crate on our position
                if field_around_agent[x][ye] == - 1:
                    break
                elif field_around_agent[x][ye] == 1:
                    bomb_targets[x-3, y-3]  += 1

    self.bomb_score = bomb_targets[2, 2]
    bomb_direction = np.array([0, 0, 0, 0, self.bomb_score])
    #oben
    above = [(1,3), (2,3), (3,3), (0,4), (1,4), (2,4), (3,4), (4,4)]
    for (x, y) in above:
        if flooded_field[x][y]== 2 and bomb_direction[0]<bomb_targets[x, y]:
            bomb_direction[0] = bomb_targets[x, y]
    #rechts
    right = [(3,1), (3,2), (3,3), (4,0), (4,1), (4,2), (4,3), (4,4)]
    for (x, y) in right:
        if flooded_field[x][y]== 2 and bomb_direction[1]<bomb_targets[x, y]:
            bomb_direction[1] = bomb_targets[x, y]
    #unten
    below = [(1,1), (2,1), (3,1), (0,0), (1,0), (2,0), (3,0), (4,0)]
    for (x, y) in below:
        if flooded_field[x][y]== 2 and bomb_direction[2]<bomb_targets[x, y]:
            bomb_direction[2] = bomb_targets[x, y]
    #links
    left = [(1,1), (1,2), (1,3), (0,0), (0,1), (0,2), (0,3), (0,4)]
    for (x, y) in left:
        if flooded_field[x][y]== 2 and bomb_direction[3]<bomb_targets[x, y]:
            bomb_direction[3] = bomb_targets[x, y]

    safety_after_bomb = flooded_field
    for x in range(7):
        safety_after_bomb[x, 3] = 0
        safety_after_bomb[3, x] = 0
    safe_fields_if_bomb_placed = np.count_nonzero(safety_after_bomb == 2)
    if safe_fields_if_bomb_placed == 0:
        bomb_direction[4] = -1
        self.badbomb = 1
    else:
        self.badbomb = 0
    if not game_state['self'][2]:
        bomb_direction = np.zeros(5) #so the agent doesnt care about bomb spots when he cant lay one
    else:
        index = np.argmax(bomb_direction)
        self.bestbombdir = index
        bomb_direction = np.zeros(5)
        bomb_direction[index] = 1
    ...
    index = np.argmax(coin_denstity_feature)

    coin_denstity_feature = np.zeros(4)
    coin_denstity_feature[index] = 1
    self.best_coin_move = np.argmax(coin_denstity_feature)


    #maybe set all features to zero if in danger

    steps_to_safety = torch.tensor(steps_to_safety, dtype=torch.float32)
    bomb_targets = torch.tensor(bomb_direction, dtype=torch.float32)
...
def Neural_network_model (self, game_state):   #input states !! 
    ...
    #target subnet
    t1 = PreLu(target_feat @ self.w_t1, self.at1)
