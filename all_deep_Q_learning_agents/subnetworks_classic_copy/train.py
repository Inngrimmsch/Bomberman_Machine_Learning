...
def reward_from_events(old_game_state, self, events: List[str]) -> int:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = { ###change all of these
        ...
        e.MOVED_UP: 1,
        e.MOVED_DOWN: 1,
        e.WAITED: -5,#*self.wc, #obviously waiting isnt inherently bad but i had the problem that agents just didnt move at all
        e.REPEATING_ONE_MOVE: -50 * self.rmp, #repeating move punishment
        e.BACK_AND_FORTH: -30 * self.bfc,
        e.INVALID_ACTION: -50*self.iac, #this should be redundant because i made invalid actions impossible
