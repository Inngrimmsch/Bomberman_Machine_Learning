...
def act(self, game_state: dict) -> str:
    ...
    Q_table = Neural_network_model(self, features)
    Prob = softmax(Q_table, dim=1)
...
def state_to_features(self, game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    for ((x,y), t) in game_state['bombs']:      
        expl[max(x-(3-t),0):min(x+(3-t), expl.shape[0]), y] += 1    #np.max and min ensure that the danger from bombs doesnt get calculatred for tiles outside of "explosion map"
        expl[x, max(y-(3-t),0):min(y+(3-t),expl.shape[1])] += 1

    close_danger_map = fov
    ...
    coins_around_agent = fov
    for (x,y) in game_state['coins']: 
        if x in range (start_x, end_x) and y in range (start_y, end_y):  
            coins_around_agent[x-start_vis_x, y-start_vis_y] = 1   #make the indices make sense ! (also for clsoe)
...
def Neural_network_model (self, features):
