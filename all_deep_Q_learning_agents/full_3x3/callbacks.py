...
def act(self, game_state: dict) -> str:
    ...
    """
    walkable_tiles = state_to_features(self, game_state)[0][0]
    change = np.array([1, 1, 1, 1, 1, 1])
    #print("walk", walkable_tiles)
    if game_state['self'][2] == False:
        change[5] = 0

    if walkable_tiles[1][0] == 0:
        change[0] = 0

    if walkable_tiles[1][2] == 0:
        change[2] = 0

    if walkable_tiles[2][1] == 0:
        change[1] = 0

    if walkable_tiles[0][1] == 0:
        change[3] = 0
    ...
    #Prob = softmax(self.Q_table, dim=0),Prob = Prob.detach().cpu().numpy(),Prob = Prob.squeeze(),out = np.random.choice(ACTIONS, p=Prob) 
    change = torch.tensor(change, dtype=torch.float32)
    prob = self.Q_table*change
...
def state_to_features(self, game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    start_vis_x = self.x-1
    end_vis_x = self.x+2   #plus 5 because when slicing the end is excluded
    start_vis_y = self.y-1
    end_vis_y = self.y+2
    ...
    for (x,y) in game_state['coins']: 
        if x in range (start_x, end_x) and y in range (start_y, end_y):  
            coins_around_agent[x-start_vis_x, y-start_vis_y] = 9 

    
    close_others = fov.copy()
    for (n,s,b,(x,y)) in game_state['others']:                 
        if x in range (start_x, end_x) and y in range (start_y, end_y):
            close_others[x-start_vis_x, y-start_vis_y] = 9

    #should make walls and coins around agent absolete obsolete
    walkable_tiles = free_tile + coins_around_agent
    bomb_targets = close_others + crate
    #print("next feat")
    #print("walls", walls)
    ...
    features = np.stack((walkable_tiles, bomb_targets, close_danger_map), axis=2)  #7x7x6 feature but 1x6x7x7 needed
...
def Neural_network_model (self, features):   #input features, not states!! 
    features = dropout(features, self.p_dropout_input).squeeze(0)
    features = features.squeeze(1)
    x = torch.reshape(features, (1, -1))
    
    h1 = PreLu(x @ self.w_h1, self.a1)
    dropout_h1 = dropout(h1, self.p_dropout_hidden)
    h2 = PreLu(dropout_h1 @ self.w_h2, self.a2)
    dropout_h2 = dropout(h2, self.p_dropout_hidden)
    h3 = PreLu(dropout_h2 @ self.w_h3, self.a3)
    dropout_h3 = dropout(h3, self.p_dropout_hidden)
    out = torch.matmul(dropout_h3, self.w_o).squeeze(0)
    return out


    #bombs and expl trennen
