...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if new_game_state['self'][3][0] - old_game_state['self'][3][0] != 0\
    or new_game_state['self'][3][1] - old_game_state['self'][3][1] != 0:
        actually_moved = True

    if last_action < 4 and int(dir2[last_action]) == 1 and actually_moved:
        events.append(MOVED_AWAY_FROM_DANGER)

    if last_action < 4 and int(dir2[last_action]) == -1 and actually_moved:
        events.append(MOVED_TO_DANGER)

    if last_action < 4 and int(dir1[last_action]) >= 1 and actually_moved:
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]): 
    ...
    if last_action < 4 and int(dir2[last_action]) == 1:
        events.append(MOVED_AWAY_FROM_DANGER)

    if last_action < 4 and int(dir2[last_action]) == -1:
        events.append(MOVED_TO_DANGER)

    if last_action < 4 and int(dir1[last_action]) >= 1:
        events.append(MOVED_TO_COINS)
