...
def setup_training(self):
    ...
    if not os.path.isfile("my-saved-model.pt"):     
        self.w_h1= init_weights((75, 128))
        self.w_h2 = init_weights((128, 256))
        self.w_h3 = init_weights((256, 128))
        self.w_o = init_weights((128, 6))
        self.a1 = init_alpha((128,))
        self.a2 = init_alpha((256,))
        self.a3 = init_alpha((128,))
...
def reward_from_events(old_game_state, self, events: List[str]) -> int:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = {
        ...
        e.COIN_COLLECTED: 70,
        e.SURVIVED_ROUND: 30,
        e.CRATE_DESTROYED: 30, #change this when training in classic scenario
        ...
        e.WAITED: -50, #obviously waiting isnt inherently bad but i had the problem that agents just didnt move at all
        e.GOT_KILLED: -100,
        e.REPEATING_ONE_MOVE: -100 * self.rmp, #repeating move punishment
        e.BACK_AND_FORTH: -500 * self.bfc,
        e.KILLED_SELF: -10000,
    }
    reward_sum = 0
    for event in events:
...
def Moved_to_safety_condition(self, old_game_state: dict, new_game_state: dict) -> bool:
    ...
    if newfeat[0, 2, 2, 2] == 0 and oldfeat[0, 2, 2, 2] != 0:
        return True
    else:
        return False
