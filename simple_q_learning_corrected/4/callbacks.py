...
def act(self, game_state: dict) -> str:
    ...
    ## To train existing model with addition of coin on next field:
    ...
    #####################################################################
    exists, location, tracer = q_rotational_symmetry(self, fake_dir1, dir2, tile, center)
    location_new = ''
    for i in range(4):
        location_new = location_new + str(int(dir1[i]))
    for i in range(4):
        location_new = location_new + str(int(dir2[i]))
    for i in range(8):
        location_new = location_new + str(int(tile[i]))
    location_new = location_new + str(int(center))
    ########################-> rework above section after first training with new feature addition
    if not exists: ## FOR OUTSIDE TRAINING ADD SIMILARITY FACTOR TO NOT MAKE RANDOM CHOICE HERE
        self.current_location = location_new
        ...
        if np.abs(tile[4]) != 0:
            initial_guess[3] = -10
        self.q_table[location_new] = initial_guess
    else:
    ...
    if self.train and random.random() < self.random_prob:
        ...
        ###### remove
        if not exists and max(dir1) == 2:
            self.q_table[location_new] = self.q_table[location].copy()
        #############
        return choice
    
    if not exists and max(dir1) == 2:
        q_values = list(self.q_table[location_new])
    else:
...
def extraction(game_state: dict, shape_vector) -> np.array:
    ...
    ## extracting the rest
    explosion_scan = game_state['explosion_map']\
    ...
    for i, j in np.ndindex(explosion_feature.shape):
        explosion_feature[j,i] = np.max(np.concatenate((explosion_scan\
            [i:i+shape_vector[0]+shape_vector[2]-1,j+shape_vector[1]-1],\
