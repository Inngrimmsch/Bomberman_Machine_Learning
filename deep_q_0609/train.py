...
Transition = namedtuple('Transition',
                        ('state', 'Q_table', 'action', 'next_state', 'reward'))
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    self.transitions.append(Transition(state_to_features(self, old_game_state), self.Q_table.squeeze(0), self_action, state_to_features(self, new_game_state), reward_from_events(old_game_state, self, events)))
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.transitions.append(Transition(state_to_features(self, last_game_state), self.Q_table, last_action, None, reward_from_events(last_game_state, self, events)))

    action_to_index = {action: i for i, action in enumerate(ACTIONS)}

    self.optimizer = optim.Adam([self.conv_w1, self.conv_w2, self.w_h1, self.w_o, self.a], lr = self.alpha)
    for t in range(last_game_state['step']):
        t_th_transition = self.transitions[t]
        action_index = action_to_index[t_th_transition.action]
        Q_pred = t_th_transition.Q_table[action_index]
        next_Q_Pred = self.transitions[t+1].Q_table
        next_Q_Pred = torch.max(next_Q_Pred)
        true_reward = t_th_transition.reward            #convince yourself that this is the right reward 
        squared_loss_function = torch.square(true_reward+self.gamma*next_Q_Pred-Q_pred)
        self.optimizer.zero_grad()
        squared_loss_function.backward()
        self.optimizer.step
    
    ####ADD THE UPDATE FOR THE LAST STEP
    last_step = last_game_state['step']
    action_index = action_to_index[self.transitions[last_step].action]
    Q_pred = self.transitions[last_step].Q_table[action_index]
    true_reward = self.transitions[last_step].reward
    squared_loss_function = torch.square(true_reward-Q_pred)
    self.optimizer.zero_grad()
    squared_loss_function.backward()
    self.optimizer.step
...
def reward_from_events(old_game_state, self, events: List[str]) -> int:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = {
        ...
        e.KILLED_OPPONENT: 5,
        e.GOT_KILLED: -5,
        e.COIN_FOUND: 0.25*(1+np.exp(-step*0.2)),
        e.KILLED_SELF: -10,
        e.INVALID_ACTION: -3,
