...
def act(self, game_state: dict) -> str:
    ...
    Q_table = self.Q_table.detach().cpu().numpy().squeeze()
...
def state_to_features(self, game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    start_vis_x = self.x-2
    end_vis_x = self.x+3   #plus 5 because when slicing the end is excluded
    start_vis_y = self.y-2
    end_vis_y = self.y+3
