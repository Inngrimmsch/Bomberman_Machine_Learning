...
def setup(self):
    ...
    if os.path.isfile("modelno0.pt"):
        ...
        self.game_number = 0
        self.q_table = {}
    self.random_prob = self.random_prob * np.exp(-self.game_number/200000)
    self.alpha = self.alpha * np.exp(-self.game_number/200000)
...
def act(self, game_state: dict) -> str:
    ...
    if not exists: ## FOR OUTSIDE TRAINING ADD SIMILARITY FACTOR TO NOT MAKE RANDOM CHOICE HERE
        ...
        for i in range(4):
            initial_guess[i] = dir1_array[i] + 2*dir2_array[i] ### EDIT CONSIDERING NEW FEATURE
        if np.abs(tile[4]) != 0:
            initial_guess[0] = -5
        if np.abs(tile[1]) != 0:
            initial_guess[1] = -5
